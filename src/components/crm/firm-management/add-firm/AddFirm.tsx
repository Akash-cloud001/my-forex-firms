  ...
  useEffect(() => {
    if (!isEditMode && isDirty) {
      const subscription = watch((value) => {
        const draftData = { formData: value, currentStep, completedSteps, timestamp: Date.now() };
  ...
  const handleSubmit = methods.handleSubmit(async (data) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSubmitStatus({ 
          type: 'error', 
          message: result.message || `Failed to ${isEditMode ? 'update' : 'create'} firm` 
        });
      }
    } catch (error) {
                    ...
                    <li key={step.id}>
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-lg transition-colors ${
                          currentStep === step.id ? 'bg-sidebar-ring text-white' :
                          completedSteps.includes(step.id) ? 'bg-primary/10 text-primary' :
                          'bg-primary/10 hover:bg-primary/10 text-white'
                        }`}
                      >
                        <div className="flex items-center space-x-3">
                          <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-medium ${
                            completedSteps.includes(step.id) ? 'bg-primary/30 text-white' :
